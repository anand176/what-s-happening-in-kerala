...
async function fetchCalendarific(year: number): Promise<FestivalItem[]> {
  ...
  if (!key) return [];
  const apiBase =
    process.env.CALENDARIFIC_API_BASE || "https://calendarific.com/api/v2/holidays";
...
async function fetchNagerDate(year: number): Promise<FestivalItem[]> {
  const apiBase =
    process.env.NAGER_DATE_BASE || "https://date.nager.at/api/v3/PublicHolidays";
...
/** Shared loader for `/api/festivals` and the homepage (avoids flaky HTTP self-fetch during SSR). */
